package object Multiplicacion {
  ...
  def PeasantAlgorithmIt(x: Int, y: Int, acc: Int = 0): Int = {
    // Version iterativa lineal del Peasant Algorithm para multiplicar dos enteros positivos
    if (x == 0) acc
    else if (x % 2 == 0) PeasantAlgorithmIt(x / 2, y * 2, acc) // Si x es par, dividir y multiplicar
    else PeasantAlgorithmIt(x / 2, y * 2, acc + y) // Si x es impar, acumular y
  }
